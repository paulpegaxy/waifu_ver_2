...
namespace WebGLSupport
{
    ...
    public class WebGLInput : MonoBehaviour, IComparable<WebGLInput>
    {
        ...
        public static event Action OnShowInput = () => { };
        public static event Action OnHideInput = () => { };

        public static event Action OnEndAndSubmit = () => { };

        public static event Action OnEndEditInput = () => { };
...
#endif
            // for mobile platform
            if (Application.isMobilePlatform || TelegramWebApp.IsMobile())
            {
        ...
        RectInt GetElemetRect()
        {
            ...
            // モバイルの場合、強制表示する
            if (showHtmlElement || Application.isMobilePlatform || TelegramWebApp.IsMobile())
            {
                var x = (int)(rect.x);
        ...
        public void OnSelect()
        {
            ...
            // モバイルの場合、強制表示する
            var isHidden = !(showHtmlElement || Application.isMobilePlatform || TelegramWebApp.IsMobile());
            id = WebGLInputPlugin.WebGLInputCreate(WebGLInput.CanvasId, rect.x, rect.y, rect.width, rect.height, fontSize, input.text, input.placeholder, input.lineType != LineType.SingleLine, isPassword, isHidden, Application.isMobilePlatform || TelegramWebApp.IsMobile());
            ...
            // WebGLWindow.OnResizeEvent += OnResizeWindow;
            WebGLWindow.OnForceBlurAndroidEvent += OnWindowForceBlurAndroid;
            WebGLWindow.OnForceBlurIOSEvent += OnWindowForceBlurIOS;
        }
        ...
        private void OnResizeWindow(bool status)
        {
            if (!status)
            {
                if (id != -1)
        ...
        private void OnWindowForceBlurAndroid()
        {
            if (id != -1)
                OnBlur(id);
        }

        private void OnWindowForceBlurIOS()
        {
        ...
        {
            if (!Application.isMobilePlatform || !TelegramWebApp.IsMobile()) return;
            if (!instances.ContainsKey(id)) return;
